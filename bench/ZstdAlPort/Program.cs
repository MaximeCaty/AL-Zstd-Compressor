...
if (files.Count == 0) { Console.WriteLine("usage: <file or directory>... [--levels Fast,Medium,Heavy] [--no-verify] [--no-ref]"); return 1; }
...
cols.AddRange(levels.Select(l => "al-" + l));
int[] refLevels = { 1, 3, 9, 19 };
if (reference) cols.AddRange(refLevels.Select(l => "zstd-" + l));

var totals = new Dictionary<string, long>();
var alStats = levels.ToDictionary(l => l, _ => new Stats());
long totalRaw = 0;
var enc = new ZstdAlEncoder(); // one instance for all files, like the SingleInstance codeunit

Console.WriteLine($"{"file",-26}{"raw",11}  " + string.Join("", cols.Select(c => $"{c,10}")) + "   (compressed size as % of raw ; al-* vs gz-opt)");
foreach (var f in files)
{
    ...
    {
        var z = enc.Compress(data, lvl);
        alStats[lvl].Add(enc.Stats);
        if (verify && !ZstdCli.Decompress(z).AsSpan().SequenceEqual(data))
            throw new Exception($"round trip failed : {f} level {lvl}");
        sizes["al-" + lvl] = z.Length;
    }
    if (reference)
    ...
    Console.WriteLine($"{Path.GetFileName(f),-26}{data.Length,11}  " + string.Join("", cols.Select(c => $"{100.0 * sizes[c] / Math.Max(1, data.Length),9:F2}%"))
        + "   " + string.Join(" ", levels.Select(l => $"{Delta(sizes["al-" + l], sizes["gz-opt"]),6}")));
}
Console.WriteLine(new string('-', 39 + 10 * cols.Count));
Console.WriteLine($"{"TOTAL",-26}{totalRaw,11}  " + string.Join("", cols.Select(c => $"{100.0 * totals[c] / totalRaw,9:F2}%"))
    + "   " + string.Join(" ", levels.Select(l => $"{Delta(totals["al-" + l], totals["gz-opt"]),6}")));
Console.WriteLine();
...
foreach (var l in levels)
{
    ...
    Console.WriteLine($"  {l,-7} ~{s.EstimatedAlMs() / mb,6:F0} ms/MB   inserts {s.Inserts / mb / 1000,7:F0}k  positions {s.Positions / mb / 1000,7:F0}k  candidates {s.Candidates / mb / 1000,7:F0}k  bytes {s.Bytes / mb / 1000,7:F0}k  (per MB)");
}
return 0;
